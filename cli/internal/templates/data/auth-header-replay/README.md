...
JSON with `measurements` containing per-endpoint round results for each user, `hashes_match` (whether responses are identical), and `body_length_delta`. No status or confidence — the AI reads measurements to classify.
