...
2. Edit `exploit.py` and fill in TARGET_URL and TOKEN (a valid JWT)
...
JSON with `schema_version: 2` and `measurements` containing original token round, no-token round, and per-variant results with `status_matches_original`, `hash_matches_original`, and `status_matches_no_token`. No status or confidence -- the AI reads measurements to classify.
